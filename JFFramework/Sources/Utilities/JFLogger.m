...
@interface JFDefaultLoggerHandle : NSObject

#pragma mark Properties

// Data
@property (strong, nonatomic)	JFLogger*	defaultLoggerObject;
@property (assign, nonatomic)	BOOL		defaultShouldLogValue;
...
// Utilities
static NSMutableDictionary*	_defaultLoggerHandles	= nil;
...
{
	JFDefaultLoggerHandle* handle = [self defaultLoggerHandle];
	@synchronized(handle)
	{
		return handle.defaultShouldLogValue;
	}
}
...
+ (void)setDefaultShouldLogValue:(BOOL)shouldLog
{
	...
	@synchronized(handle)
	{
		handle.defaultShouldLogValue = shouldLog;
	}
}
...
+ (JFLogger*)defaultLogger
{
	...
	@synchronized(handle)
	{
		return handle.defaultLoggerObject;
	}
}
...
{
	if(!logger)
		logger = [JFLogger defaultLogger];
	
	JFDefaultLoggerHandle* handle = [self defaultLoggerHandle];
	@synchronized(handle)
	{
		handle.defaultLoggerObject = logger;
	}
}

+ (JFDefaultLoggerHandle*)defaultLoggerHandle
{
	NSString* key = ClassName;
	@synchronized(_defaultLoggerHandles)
	{
		JFDefaultLoggerHandle* retObj = [_defaultLoggerHandles objectForKey:key];
		if(!retObj)
		{
			retObj = [[JFDefaultLoggerHandle alloc] init];
			retObj.defaultLoggerObject = [JFLogger defaultLogger];
			retObj.defaultShouldLogValue = YES;
			[self setDefaultLoggerHandle:retObj];
		}
		return retObj;
	}
}

+ (void)setDefaultLoggerHandle:(JFDefaultLoggerHandle*)handle
{
	NSString* key = ClassName;
	@synchronized(_defaultLoggerHandles)
	{
		if(handle)
			[_defaultLoggerHandles setObject:handle forKey:key];
		else
			[_defaultLoggerHandles removeObjectForKey:key];
	}
}
...
+ (void)load
{
	...
	dispatch_once(&token, ^{
		_defaultLoggerHandles = [NSMutableDictionary new];
	});
}
...
@implementation JFDefaultLoggerHandle

#pragma mark Properties

// Data
@synthesize defaultLoggerObject		= _defaultLoggerObject;
@synthesize defaultShouldLogValue	= _defaultShouldLogValue;
